...
function BookForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(`http://127.0.0.1:5000/api/books/${id}`, bookData, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ ...commonStyles.title, textAlign: 'center' }}>{id ? 'Edit Book' : 'Add New Book'}</h2>
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <button type="submit" style={commonStyles.submitButton}>Save Book</button>
          </form>
        )}
